...
rm -f $HOME_PATH/helm-charts/starrocks-resource-manager/values.yaml
cat <<EOF >$HOME_PATH/helm-charts/starrocks-resource-manager/values.yaml
...
operator:
`awk '{printf("  %s\n", $0);}' $HOME_PATH/helm-charts/starrocks-resource-manager/charts/operator/values.yaml`

manager:
`awk '{printf("  %s\n", $0);}' $HOME_PATH/helm-charts/starrocks-resource-manager/charts/manager/values.yaml`
EOF
